...
In this project, we will explore two different datasets using multiple linear regression, ridge regression, lasso regression, transformed regression (specifically Sqrt, Log1p, and Box-Cox), second-order polynomial regression including interaction terms, neural networks (two, three, and four layers), and random forests. The two datasets we will be using are:

\begin{itemize}
	\item \textbf{Insurance Charges (Data Set 1):} This dataset contains information about different primary beneficiaries of health insurance. The goal is to predict the primary beneficiaries insurance charges given their other characteristics. The dataset can be found at \url{https://www.kaggle.com/datasets/mirichoi0218/insurance}.
	\item \textbf{Insurance Annual Medical Cost (Data Set 2):} This dataset contains information about different primary beneficiaries of health insurance. The goal is to predict the primary beneficiaries annual medical cost given their other characteristics. The dataset can be found at\\ \url{https://www.kaggle.com/datasets/mohankrishnathalla/medical-insurance-cost-prediction}.
\end{itemize}
...
We optimized the neural network's activation functions, hidden layer sizes, and learning rate using a sequential grid search, maximizing the mean $R^2$ via 5-fold cross-validation. First, activation functions were tuned layer-by-layer, from the initial hidden layer through to the output. We then determined the hidden layer sizes; for two-layer architectures, we constrained the second layer to be less than or equal to the first. Finally, the learning rate was tuned.

\section{Insurance Charges (Data Set 1)}\label{Insurance Charges}
...
\begin{figure}[H]
     \centering
     \includegraphics[width=0.5\textwidth]{../Insurance_Charges/corrmatrix.png}
     \caption{Insurance Charges Correlation Matrix}
     \label{fig:Insurance Charges Correlation Matrix}
\end{figure}

\begin{figure}[H]
     \centering
     \includegraphics[width=0.5\textwidth]{../Insurance_Charges/rfregressionoos.png}
     \caption{Insurance Charges Random Forrest Out-of-Sample Plot\\ black: actual y-values, red: predicted y-values}
     \label{fig:Insurance Charges Random Forrest Out-of-Sample Plot}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Insurance Charges Random Forest Regression metrics}
\label{tab:Insurance Charges Random Forest Regression metrics}
%\resizebox{\textwidth}{!}{
\begin{tabular}{|c|c|}\hline
Metric & Random Forest  \\ \hline \hline
rSq & 0.8225083593592852 \\ \hline
rmse & 5056.020242853598 \\ \hline
mae & 2956.7923371477714 \\ \hline
\end{tabular}
%}
\end{table}
...
\begin{table}[H]
\centering
\caption{Insurance Charges Random Forest Regression Ytest values}
\label{tab:Insurance Charges Random Forest Regression Ytest values}
%\resizebox{\textwidth}{!}{
\begin{tabular}{|c|c|}\hline
Metric & Random Forest \\ \hline \hline
rSq & 0.8225083593592852  \\ \hline
sst & 154251421569.71732 \\ \hline
sse & 27378337885.571674 \\ \hline
mse0 & 25563340.696145356  \\ \hline
rmse & 5056.020242853598 \\ \hline
mae & 2956.7923371477714 \\ \hline
m & 1071 \\ \hline
\end{tabular}
...
Hyper Parameter tuning info:

Best params:

{n estimators: 700, min samples split: 2, min samples leaf: 1, max features: 0.5, max depth: 10, bootstrap: False}

\begin{table}[H]
\centering
\caption{Insurance Charges Random Forest Regression Tuned Hyper Parameters}
\label{tab:Insurance Charges Random Forest Regression Tuned Hyper Parameters}
%\resizebox{\textwidth}{!}{
\begin{tabular}{|c|c|}\hline
n estimators & 700  \\ \hline \hline
min samples split & 2 \\ \hline
min samples leaf & 1 \\ \hline
max features & 0.5 \\ \hline
max depth & 10 \\ \hline
...
\begin{table}[H]
\centering
\caption{Insurance Charges Random Forest Regression Tuned metrics}
\label{tab:Insurance Charges Random Forest Regression Tuned metrics}
%\resizebox{\textwidth}{!}{
\begin{tabular}{|c|c|}\hline
Metric & Random Forest  \\ \hline \hline
rSq & 0.8164419175052696 \\ \hline
rmse & 5141.698517661322 \\ \hline
mae & 3087.9428686341853 \\ \hline
\end{tabular}
%}
\end{table}
...
\begin{figure}[H]
     \centering
     \includegraphics[width=0.5\textwidth]{../Insurance_Charges/featureimportance.png}
     \caption{Insurance Charges Feature Importance}
     \label{fig:Insurance Charges Feature Importance}
\end{figure}
...
\section{Insurance Annual Medical Cost (Data Set 2)}

\subsection{Exploratory Data Analysis}

The Insurance Annual Medical Cost dataset contains $100,000$ observations each having $54$ variables. Some of the variables are:
\begin{itemize}
	\item age: The age of the primary beneficiary.
	\item sex: The insurance contractor sex: female or male.
	\item bmi: The body mass index of the primary beneficiary.
	\item dependents: The number of dependents covered by health insurance
	\item smoker: If the primary beneficiary smokes.
	\item region: The beneficiary's residential area in the US.
	\item claims\_count: Number of claims submitted to insurance.
	\item avg\_claim\_amount: Average amount of the submitted claims.
	\item annual\_medical\_cost: (The target variable) Individual medical costs billed by health insurance.
\end{itemize}

The Insurance Annual Medical Cost dataset contains 30,083 missing values, so we remove these rows. For modeling purposes, we converted all the categorical variables into dummy variables.

Univariate distributions are visualized in \Cref{fig:Insurance Annual Medical Cost Histograms} (histograms/count-plots) and \Cref{fig:Insurance Annual Medical Cost Box Plots} (box plots for continuous variables).

%The visualizations reveal that sex and region are approximately uniformly distributed. While age also follows a generally uniform pattern, there is a notable spike in frequency for individuals aged $18$ and $19$. Additionally, bmi closely follows a normal distribution, whereas children and charges exhibit distributions characteristic of a geometric decay. Notably, both bmi and charges contain several outliers, with the latter displaying a significantly higher concentration. Finally, non-smokers represent the most prevalent group in the dataset.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.75\textwidth]{../Insurance_Annual_Medical_Cost/histograms.png} 
    \caption{Insurance Annual Medical Cost Histograms and Count-Plots}
    \label{fig:Insurance Annual Medical Cost Histograms}
\end{figure}

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.75\textwidth]{../Insurance_Annual_Medical_Cost/boxplots.png} 
    \caption{Insurance Annual Medical Cost Box Plots}
    \label{fig:Insurance Annual Medical Cost Box Plots}
\end{figure}

\Cref{fig:Insurance Annual Medical Cost Scatterplots} displays the bivariate relationships between some of the predictors and the target variable, Annual Medical Cost, while the corresponding correlation matrix is provided in \Cref{fig:Insurance Annual Medical Cost Correlation Heat Map}.

%Analysis of these figures indicates that the smoker variable has the strongest correlation with charges. In the scatterplot for age, the data points appear to bifurcate into three distinct clusters, each forming its own linear trend. This suggests that latent variables or interaction effects (currently unobserved) might differentiate these groups, as each subgroup shows a strong individual correlation with charges. Furthermore, the figures reveal that the remaining predictor variables exhibit no discernible relationship with charges, appearing as randomly distributed clusters with no clear linear or non-linear patterns. Finally, there is no evidence of significant multi-collinearity or inter-correlation between any of the predictor variables.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.75\textwidth]{../Insurance_Annual_Medical_Cost/scatterplots.png} 
    \caption{Insurance Annual Medical Cost Scatterplots}
    \label{fig:Insurance Annual Medical Cost Scatterplots}
\end{figure}

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.5\textwidth]{../Insurance_Annual_Medical_Cost/correlation_heatmap.png} 
    \caption{Insurance Annual Medical Cost Correlation Heat Map}
    \label{fig:Insurance Annual Medical Cost Correlation Heat Map}
\end{figure}

\Cref{fig:Insurance Annual Medical Cost Hue Plots} illustrates the bivariate relationships among some of the predictor variables, with data points colored by the target variable, Annual Medical Cost.

%Given the high proportion of categorical predictors, many of these visualizations result in overlapping clusters, making it difficult to definitively determine if most interaction terms would improve model performance. A primary exception is the interaction between bmi and smoker, where it is very clear that the interaction is beneficial. In contrast, for variables such as children and sex, the influence on the target variable remains obscured by the data's density, leaving their potential for interaction-based improvement unclear.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=0.9\textwidth]{../Insurance_Annual_Medical_Cost/Hue_Plots.png} 
    \caption{Insurance Annual Medical Cost Hue Plots}
    \label{fig:Insurance Annual Medical Cost Hue Plots}
\end{figure}
...
\subsection{Order 2 Polynomial}

Insurance Annual Medical Cost Order 2 Polynomial alpha Used: 0.0015

\begin{figure}[H]
     \centering
         \includegraphics[width=0.5\textwidth]{../Insurance_Annual_Medical_Cost_Plots/Order2Reg_80_20.png}
     \caption{Insurance Annual Medical Cost Order 2 Polynomial Out-of-Sample\\ black: actual y-values, red: predicted y-values}
     \label{fig:Insurance Annual Medical Cost Order 2 Polynomial Plots}
\end{figure}

\begin{table}[H]
\centering
\caption{Insurance Annual Medical Cost Order 2 Regression CV}
\label{tab:Insurance Annual Medical Cost Order 2 Regression CV}
\begin{tabular}{|c|c|c|c|c|c|}\hline
Metric & num folds & min & max & mean & stdev \\ \hline \hline
rSq & 5 & 0.5883 & 0.659 & 0.6347 & 0.02461 \\ \hline
rSqBar & 5 & 0.5858 & 0.6568 & 0.6325 & 0.02476 \\ \hline
sst & 5 & 1.857e+11 & 2.067e+11 & 1.956e+11 & 8.375e+09 \\ \hline
sse & 5 & 6.751e+10 & 7.713e+10 & 7.131e+10 & 3.274e+09 \\ \hline
sde & 5 & 1843 & 1970 & 1894 & 43.13 \\ \hline
mse0 & 5 & 3.376e+06 & 3.856e+06 & 3.565e+06 & 1.637e+05 \\ \hline
rmse & 5 & 1837 & 1964 & 1888 & 43 \\ \hline
mae & 5 & 1105 & 1138 & 1123 & 11.7 \\ \hline
smape & 5 & 43.71 & 44.92 & 44.48 & 0.4484 \\ \hline
m & 5 & 2e+04 & 2e+04 & 2e+04 & 0 \\ \hline
dfr & 5 & 124 & 124 & 124 & 0 \\ \hline
df & 5 & 1.988e+04 & 1.988e+04 & 1.988e+04 & 0 \\ \hline
fStat & 5 & 229.1 & 309.7 & 280.4 & 27.76 \\ \hline
aic & 5 & 3.009e+05 & 3.036e+05 & 3.02e+05 & 903.8 \\ \hline
bic & 5 & 3.019e+05 & 3.045e+05 & 3.029e+05 & 903.8 \\ \hline
\end{tabular}
\end{table}
...
Note, do the time restraints, we did not rigorously tune the hyperparamters for the neural network.

Insurance Annual Medical Cost NN 4L hidden activation 1: ELU, hidden activation 2: ELU, output activation: Identity, hidden 1: 128, hidden 2: 64, lr: 5e-03

\begin{figure}[H]
     \centering
         \includegraphics[width=0.5\textwidth]{../Insurance_Annual_Medical_Cost_Plots/NN_4L_80_20.png}
     \caption{Insurance Annual Medical Cost 4 Layer Neural Network Out-of-Sample\\ black: actual y-values, red: predicted y-values}
     \label{fig:Insurance Annual Medical Cost 4 Layer Neural Network Plots}
\end{figure}

\begin{table}[H]
\centering
\caption{Insurance Annual Medical Cost 4 Layer Neural Network CV}
\label{tab:Insurance Annual Medical Cost 4 Layer Neural Network CV}
\begin{tabular}{|c|c|c|c|c|c|}\hline
Metric & num folds & min & max & mean & stdev \\ \hline \hline
rSq & 5 & 0.6168 & 0.6883 & 0.662 & 0.02496 \\ \hline
rSqBar & 5 & 0.6166 & 0.6881 & 0.6618 & 0.02498 \\ \hline
sst & 5 & 1.857e+11 & 2.067e+11 & 1.956e+11 & 8.375e+09 \\ \hline
sse & 5 & 6.338e+10 & 7.179e+10 & 6.594e+10 & 3.081e+09 \\ \hline
sde & 5 & 1781 & 1895 & 1816 & 41.9 \\ \hline
mse0 & 5 & 3.169e+06 & 3.589e+06 & 3.297e+06 & 1.541e+05 \\ \hline
rmse & 5 & 1780 & 1895 & 1815 & 41.88 \\ \hline
mae & 5 & 1007 & 1069 & 1036 & 21.76 \\ \hline
smape & 5 & 37.28 & 39.96 & 38.08 & 1.01 \\ \hline
m & 5 & 2e+04 & 2e+04 & 2e+04 & 0 \\ \hline
dfr & 5 & 13 & 13 & 13 & 0 \\ \hline
df & 5 & 1.999e+04 & 1.999e+04 & 1.999e+04 & 0 \\ \hline
fStat & 5 & 2475 & 3396 & 3035 & 318.8 \\ \hline
aic & 5 & 2.994e+05 & 3.019e+05 & 3.002e+05 & 911.2 \\ \hline
bic & 5 & 2.995e+05 & 3.02e+05 & 3.003e+05 & 911.2 \\ \hline
\end{tabular}
\end{table}
...
\subsubsection{In-Sample Quality of Fit Metrics}
...
\subsubsection{Out-of-Sample Quality of Fit Metrics}
\begin{table}[H]
\centering
...
\subsubsection{Fine-Tuned Quality of Fit Metrics}
...
\subsubsection{Feature Importance from the Best-Performing Model}
...
When modeling the medical insurance costs, our first data set indicates that the beneficiaries smoker status and bmi (and their interaction) are extremely informative variables. Meanwhile, our second data set indicates that these features are not informative, and rather the most informative variables are average claim amount and claims count. This is most likely due to how each insurance company decides premiums. Some may choose to vary the premium greatly based on factors such as smoker status and bmi, while others may give everyone a similar base rate that is only affected by claims.

For modeling, Order 2 Polynomials will provide a strong interpretable model, while Neural Networks provide the best results but lose the interpretability, and Random Forests offers a middle ground.
